...
logging.getLogger("httpcore").setLevel(
    os.environ.get("HTTPCORE_LOGLEVEL", "ERROR").upper()
)
logging.getLogger("openai._base_client").setLevel(
    os.environ.get("OPENAI_BASE_CLIENT_LOGLEVEL", "WARNING").upper()
)
logging.getLogger("urllib3").setLevel(
    os.environ.get("URLLIB3_LOGLEVEL", "WARNING").upper()
)
logging.getLogger("urllib3.connectionpool").setLevel(
    os.environ.get("URLLIB3_CONNECTIONPOOL_LOGLEVEL", "WARNING").upper()
)
logging.getLogger("openai").setLevel(
    os.environ.get("OPENAI_LOGLEVEL", "WARNING").upper()
)
logging.getLogger("autogen_core").setLevel(
    os.environ.get("AUTOGEN_CORE_LOGLEVEL", "WARNING").upper()
)
logging.getLogger("autogen_core.events").setLevel(
    os.environ.get("AUTOGEN_EVENTS_LOGLEVEL", "WARNING").upper()
)
...
async def orc(req: Request) -> JSONResponse:
    ...
    if question:
        orchestrator = RequestResponseOrchestrator(
            thread_id, OrchestratorConfig(), client_principal, access_token
        )
        result = await orchestrator.answer(question)
        return JSONResponse(content=result)
    else:
        return JSONResponse(
            content={"error": "no question found in json input"}, status_code=400
        )
