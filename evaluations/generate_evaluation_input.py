...
- thread_id (str): UUID extracted from the response or reused from the input.
...
Functions:
----------
- extract_thread_id_from_chunk(chunk: str) -> tuple[str | None, str]:
    Extracts a UUID conversation ID from the start of a text chunk, if present.

- send_question_to_rest_api(uri: str, function_key: str, question: str, thread_id: str) -> tuple[str | None, str]:
...
def extract_thread_id_from_chunk(chunk: str):
    """
    ...
    Returns:
        tuple: (thread_id (str or None), cleaned_chunk (str))
    """
...
def send_question_to_rest_api(uri, function_key, question, thread_id):
    """
    ...
    Args:
        ...
        thread_id (str): The conversation identifier to send (may be empty).

    Returns:
        tuple: (extracted_thread_id (str or None), complete text response (str))
    """
    headers = {"x-functions-key": function_key, "Content-Type": "application/json"}
    payload = {"thread_id": thread_id, "question": question}
    try:
        ...
        for chunk in response.iter_lines(decode_unicode=True):
            if chunk:
                ...
                if extracted_conv_id is None:
                    extracted_conv_id, chunk = extract_thread_id_from_chunk(chunk)
                result_text += chunk + "\n"
...
def main():
    ...
    parser.add_argument(
        "--input",
        type=str,
        default="evaluations/input/test-dataset.jsonl",
        help="Path to the input JSONL file.",
    )
    parser.add_argument(
        "--output-folder",
        type=str,
        default="evaluations/output/",
    ...
    if not endpoint or not function_key:
        print(
            "Environment variables ORCHESTRATOR_STREAM_ENDPOINT and FUNCTION_KEY must be set."
        )
        sys.exit(1)
    ...
    with open(input_file, "r", encoding="utf-8") as fin, open(
        ...
        for line in fin:
            ...
            # Update last_thread_id if a new one was extracted.
            if extracted_conv_id:
                last_thread_id = extracted_conv_id

            record["response"] = response_text
            record["context"] = ""
            record["thread_id"] = (
                extracted_conv_id if extracted_conv_id else payload_conv
            )
